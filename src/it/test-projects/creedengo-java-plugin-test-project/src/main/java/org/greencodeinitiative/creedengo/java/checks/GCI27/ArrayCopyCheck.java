...
class TestClass {
	...
	public void copyWithForLoop() {
		...
		for (int i = 0; i < len; i++) { // Compliant (because of presence of functionnal "if")
			if(i + 2 < len) {
				dest[i] = src[i + 2];
			}
		}
		...
		for (int i = 0; i < len; i++) { // Compliant (because of presence of functionnal "try")
			try {
				dest[i] = src[i];
	...
	public void copyWithForEachLoop() {
		...
		i = -1;
		for (boolean b : src) { // Compliant (because of presence of functionnal "if")
			if(b) {
				dest[++i] = b;
			}
		}
		...
		// Copy nested by try/catch
		i = -1;
		for (boolean b : src) { // Compliant (because of presence of functionnal "try")
			try {
				dest[++i] = b;
			} catch (RuntimeException e) {
				e.printStackTrace();
	...
	public void copyWithWhileLoop() {
		...
		// Copy with nested conditions
		i = 0;
		while (i < len) { // Compliant (because of presence of functionnal "if")
			if(i + 2 < len) {
				dest[i] = src[i + 2];
			}
			i++;
		...
		// Copy nested by try/catch and if
		i = 0;
		while (i < len) { // Compliant (because of presence of functionnal "try")
			try {
				if(dest != null) {
					dest[i] = src[i];
				}
	...
	public void copyWithDoWhileLoop() {
		...
		// Copy with nested conditions
		i = 0;
		do { // Compliant (because of presence of functionnal "if")
			if(i + 2 < len) {
				dest[i] = src[i + 2];
			}
			i++;
		...
		// Copy nested by try/catch and if
		i = 0;
		do { // Compliant (because of presence of functionnal "try")
			try {
				if(dest != null) {
					dest[i] = src[i];
				}
